####input two psl files, output all matches of query file in 1.psl and 2.psl, print full line of 1.psl, and qname, qsize, qstart, qend, tname, tstart, tend of 2.psl in new file.

def pull_data():
        data = []
        peaks = []
        with open("Ymale.all.tissues.filtered.converted.psl", 'r') as macs_data:
...
def pull_data2():
        data = []
        XY = []
        with open("XIXmale.all.tissues.filtered.converted.psl", 'r') as macs_data:
                for line in macs_data:
                        if line.startswith("#"):
                                continue
                        else:
...
def compare1():
        ...
        x=0
        with open("YandXtranscripts", 'a') as out:
                while x < len(bmatch):
                        for index,value in enumerate(match):
                                if str(Qname[index]) == str(bQname[x]):
                                        out.write(str(match[index]) + '\t' + str(mismatch[index]) + '\t' + str(repmatch[index]) + '\t' + str(Ns[index]) + '\t' + str(Qgap[index]) + '\t' + str(qgapbases[index]) + '\t' + str(Tgapcount[index]) + '\t' + str(Tgapbases[index]) + '\t' + str(strand[index]) + '\t' + str(Qname[index]) + '\t' + str(Qsize[index]) + '\t' + str(Qstart[index]) + '\t' + str(Qend[index]) + '\t' + str(Tname[index]) + '\t' + str(Tsize[index]) + '\t' + str(Tstart[index]) + '\t' + str(Tend[index]) + '\t' + str(bmismatch[x]) + '\t' + str(bQname[x]) + '\t' + str(bQsize[x]) + '\t' +  str(bQstart[x]) + '\t' + str(bQend[x]) + '\t' + str(bTname[x]) + '\t' + str(bTstart[x]) + '\t' + str(bTend[x]) + '\n')
                        x+=1
